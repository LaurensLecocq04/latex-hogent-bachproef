...
\begin{itemize}
  \item \textbf{IPAM} (\emph{IP Address Management}): het beheer van de IP-adresruimte, subnetten en VLAN's binnen een netwerk.
  \item \textbf{DCIM} (\emph{Data Center Infrastructure Management}): het beheer van fysieke en logische infrastructuurelementen zoals racks, devices, kabels en interfaces.
  \item \textbf{NMS} (\emph{Network Management System}): platformen die netwerken bewaken en beheren, zoals SolarWinds NPM\autocite{SolarWindsNPM}, ManageEngine OpManager\autocite{ManageEngineOpManager} of LibreNMS\autocite{LibreNMS}.
  \item \textbf{ITSM} (\emph{IT Service Management}): platformen voor IT-dienstenbeheer en CMDB-functionaliteit, zoals ServiceNow Discovery\autocite{ServiceNowDiscovery}.
  \item \textbf{NetBox}: een open-source IPAM/DCIM-platform dat in deze bachelorproef centraal staat als \emph{single source of truth} (SSOT) voor netwerkdocumentatie\autocite{NetBoxOSS}.
  \item \textbf{NetBox Discovery}: een uitbreiding op NetBox die automatische netwerkdetectie via SNMP en NMAP-scans uitvoert via discovery-jobs en de resultaten in NetBox synchroniseert\autocite{NetBoxDiscoveryDocs}.
  \item \textbf{Diode}: een synchronisatielaag van NetBox Labs die discovery-data via gRPC en een client/server-architectuur valideert en in NetBox laadt\autocite{DiodeDocs}.
  \item \textbf{Orb agent}: een lichtgewicht discovery-agent (container) van NetBox Labs die de eigenlijke discovery uitvoert en de resultaten via Diode aanlevert aan NetBox\autocite{NetBoxLabsOrbAgent}.
  \item \textbf{LLDP} (\emph{Link Layer Discovery Protocol}): een vendor-neutraal Layer 2-protocol waarmee netwerkapparaten hun directe buren ontdekken; cruciaal voor het automatisch reconstrueren van kabel- en topologierelaties\autocite{LLDP_IJCA}.
  \item \textbf{SNMP} (\emph{Simple Network Management Protocol}): een protocol voor het uitlezen van device-attributen en interfaceinformatie van netwerkapparatuur\autocite{SnmpTopologyDiscoveryIEEE2013}.
\end{itemize}
...
Hoofdstuk~\ref{ch:casestudy} beschrijft de casestudy. Voor elke geëvalueerde site (Ridefort, Deleers Go4tech, Koninklijke Stallingen en WZC Riethove) wordt de baseline samen met de na-meting gepresenteerd in vergelijkbare voor/na-tabellen.

Hoofdstuk~\ref{ch:analyse} bevat de analyse van de resultaten en de aanbevelingen voor Citymesh. Hoofdstuk~\ref{ch:conclusie} sluit het werk af met een beknopte conclusie en een korte vooruitblik op verder onderzoek.
