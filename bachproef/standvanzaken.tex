...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[site] (s1) {Site~1};
    \node[site, below=of s1] (s2) {Site~2};
    \node[site, below=of s2] (s3) {Site~$n$};

    \node[agent, anchor=south west, xshift=4mm, yshift=4mm] at (s1.north west) {Orb};
    \node[agent, anchor=south west, xshift=4mm, yshift=4mm] at (s2.north west) {Orb};
    \node[agent, anchor=south west, xshift=4mm, yshift=4mm] at (s3.north west) {Orb};

    \node[below=3mm of s3, font=\footnotesize, text=black!70] {Netwerken met discovery-agents};

    \node[diode, right=24mm of s2] (diode) {Diode\\\scriptsize\mdseries ingestion service};
    \node[netbox, right=of diode] (netbox) {NetBox\\\scriptsize\mdseries (SSOT)};

    \draw[flow] (s1.east) -- (diode.west);
    \draw[flow] (s2.east) -- node[flowlabel] {observed state} (diode.west);
    \draw[flow] (s3.east) -- (diode.west);

    \draw[flow] (diode.east) -- node[flowlabel] {gereconcilieerde wijzigingen} (netbox.west);
  \end{tikzpicture}
