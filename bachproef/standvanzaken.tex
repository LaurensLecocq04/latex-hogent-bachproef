...
\begin{itemize}
  \item ICMP/NMAP-scans: een eerste sweep om live hosts te identificeren in een subnet of IP-range. Dit is goedkoop, vendor-neutraal en typisch de eerste stap van elke discoveryflow.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    site/.style={
      draw=hogent-lightgreen!60!black,
      rounded corners=3pt,
      fill=hogent-lightgreen!40,
      minimum width=2.6cm,
    ...
    agent/.style={
      draw=hogent-orange!70!black,
      rounded corners=2pt,
      fill=hogent-orange!90,
      minimum width=1.4cm,
    ...
    netbox/.style={
      draw=hogent-darkgreen!70!black,
      rounded corners=3pt,
      fill=hogent-darkgreen!90,
      minimum width=2.8cm,
      minimum height=1.6cm,
      align=center,
      font=\small\bfseries,
    ...
    \node[site] (s1) {Site~1};
    \node[site, below=16mm of s1] (s2) {Site~2};
    \node[site, below=16mm of s2] (s3) {Site~$n$};

    \node[agent] at (s1.north) {Orb};
    \node[agent] at (s2.north) {Orb};
    \node[agent] at (s3.north) {Orb};

    \node[below=8mm of s3, font=\footnotesize, text=black!70, align=center]
      {Netwerken met discovery-agents};

    \node[diode, right=40mm of s2] (diode)
      {Diode\\[1pt]\scriptsize\mdseries ingestion service};
    \node[netbox, right=40mm of diode] (netbox)
      {NetBox\\[1pt]\scriptsize\mdseries (SSOT)};

    \draw[flow] (s1.east) -- (diode.west);
    \draw[flow] (s2.east) -- node[flowlabel] {observed\\state} (diode.west);
    \draw[flow] (s3.east) -- (diode.west);

    \draw[flow] (diode.east)
      -- node[flowlabel] {gereconcilieerde\\wijzigingen} (netbox.west);
  \end{tikzpicture}
  \caption[Architectuur van de NetBox Discovery-stack.]{Architectuur van de NetBox Discovery-stack: Orb-agents draaien per netwerksegment en sturen hun observed state via Diode door naar NetBox, waar de wijzigingen gereconcilieerd worden met de bestaande SSOT. Eigen schema, gebaseerd op de architectuurfiguur in\autocite{NetBoxDiscoveryAgentPreview}.}
  \label{fig:netbox-discovery-stack}
\end{figure}
...
\begin{itemize}
  \item network-discovery: gebruikt NMAP om IP-adressen, IP-ranges en subnets te scannen op live hosts en open poorten. Het is de eerste stap om uit te vissen welke devices bereikbaar zijn.
  \item device-discovery / SNMP-discovery: gebruikt SNMP en device-drivers om bevestigde devices verder te bevragen op hun identiteit, interfaces, IP-adressen en (waar beschikbaar) LLDP-buren.
\end{itemize}

NetBox Discovery werkt op basis van policies: configuratie-objecten die per device of per subnet beschrijven hoe en met welke frequentie discovery uitgevoerd moet worden, met welke credentials, en welke standaardwaarden (site, role, tenant) toegekend mogen worden. Discovery-jobs worden periodiek (bijvoorbeeld via een cron-expressie) door een agent uitgevoerd. Wanneer een apparaat wordt ontdekt, controleert het systeem of het al bestaat in NetBox; bestaande devices worden bijgewerkt, nieuwe devices worden aangemaakt.

\subsection{Diode als ingest-laag}

Diode is de ingest-laag tussen de discovery-agents en NetBox \autocite{DiodeDocs,NetBoxDiodeAgent}. Het is een open-source service die discoverydata via een gRPC-API ontvangt, valideert tegen het NetBox-datamodel en in NetBox laadt. Diode lost een aantal concrete problemen op die ontstaan wanneer agents rechtstreeks naar de NetBox-API zouden schrijven: (1) discovery-output is vaak partial en vereist diff-/merge-logica, (2) gelijktijdige writes door meerdere agents kunnen race conditions veroorzaken, en (3) credentials voor NetBox moeten anders centraal beschikbaar zijn op elke agent. Door Diode tussen agents en NetBox te plaatsen, wordt de schrijflaag gecentraliseerd en gemonitored.

Diode werkt met OAuth2-credentials (een \texttt{client\_id} en \texttt{client\_secret} per agent) en houdt voor elke wijziging die naar NetBox wordt geschreven bij welke agent die heeft aangeleverd en op welk moment dat gebeurde. Daardoor kan bij problemen achteraf worden nagegaan welke agent welke wijzigingen heeft veroorzaakt.

\subsection{Orb agent}

De Orb agent is het programma dat de eigenlijke netwerkscans uitvoert\autocite{NetBoxLabsOrbAgent}. NetBox Labs levert hem als één Docker-container, die per site of per netwerksegment wordt geïnstalleerd op een gewone server in dat netwerk. Bij het opstarten leest de agent een YAML-configuratiebestand waarin staat welke IP-ranges hij mag scannen en welke SSH- en SNMP-credentials hij mag gebruiken om apparaten te bevragen. Op basis daarvan voert de agent achtereenvolgens drie soorten scans uit: hij zoekt eerst naar bereikbare hosts in een IP-range (\texttt{network\_discovery}), bevraagt elke gevonden host vervolgens naar zijn naam, type en interfaces (\texttt{device\_discovery}), en haalt ten slotte via SNMP aanvullende informatie op (\texttt{snmp\_discovery}). De resultaten worden niet rechtstreeks naar NetBox geschreven, maar via Diode aangeleverd. Omdat de agent in hetzelfde netwerk draait als de apparaten die hij moet ontdekken, kan hij ook apparaten bereiken die vanuit het centrale NetBox-platform niet rechtstreeks aanspreekbaar zijn. Dat is essentieel voor de gedistribueerde, draadloze sites van Citymesh.
...
\begin{itemize}
  ...
  \item Native integratie met NetBox als SSOT.
...
Limitaties die in de praktijk relevant bleken:
\begin{itemize}
  \item De LLDP-buren die de Orb agent verzamelt, worden binnen de standaard discoveryflow met Orb en Diode niet automatisch als kabelobjecten in NetBox aangemaakt. Voor volledige topologie-discovery is een aanvullende stap nodig (bijvoorbeeld via een eigen tool die de NetBox API aanstuurt). Dit was een belangrijke vaststelling tijdens de evaluatie van deze bachelorproef.
  \item Vendor-dekking is afhankelijk van de beschikbare device-drivers in de Orb agent. Voor minder courante vendors of voor devices die alleen via een eigen API toegankelijk zijn (bijvoorbeeld draadloze controllers zoals RUCKUS Virtual Smartzone\autocite{RuckusVSZ}), zijn aanvullende koppelingen nodig.
...
De vergelijking toont vooral dat de besproken alternatieven elk een eigen zwaartepunt hebben (vendor-templates bij Slurp'it, een eigen IPAM/DCIM-platform bij Nautobot, monitoring bij LibreNMS, OpenNMS en SolarWinds NPM, en ITSM-gerichte discovery bij ServiceNow Discovery) en inhoudelijk dus niet \'e\'en-op-\'e\'en overlappen met de NetBox Discovery-stack. Dit overzicht dient hier om de bredere markt te kaderen; in het verdere onderzoek wordt de NetBox Discovery-stack zelf gebruikt, omdat NetBox bij Citymesh reeds als SSOT in gebruik is.
