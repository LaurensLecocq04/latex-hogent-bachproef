...
In de onderstaande YAML- en shellfragmenten staat de site-naam in NetBox als \texttt{Oostkamp Ridefort}; in de hoofdtekst wordt deze site kortweg als \emph{Ridefort} aangeduid. Credentials zoals \texttt{DEVICE\_USERNAME}, \texttt{DEVICE\_PASSWORD} en de SNMP-community \texttt{public} dienen in deze listings als \textbf{placeholders} en worden in productie via een secret manager respectievelijk een niet-publieke community vervangen.
...
\subsection{Aanvulling: \texttt{NetworkTool} en NetBox API}

De \texttt{NetworkTool}-toepassing, opgebouwd met een Streamlit-gebaseerde GUI, ondersteunt het discoveryproces met een modulaire opbouw voor het beheren van discovery-agents, het uitvoeren van live dataverzameling, utility-functies en NetBox-synchronisatie. Daardoor blijft de werking beheersbaar en kunnen extra workflows toegevoegd worden.

\begin{itemize}
  \item \textbf{GUI-laag:} beheert de gebruikersflow en tabs.
  \item \textbf{Dataverzameling:} haalt live data op per vendor, zoals MikroTik, Huawei en Fortinet.
  \item \textbf{NetBox-integratie:} voert dry-run analyse en effectieve synchronisatie uit.
  \item \textbf{Vendor-specifieke scripts:} voorzien exports en verwerking per ondersteunde leverancier.
\end{itemize}

\subsubsection{GUI-architectuur}

De vernieuwde GUI bevat vier tabs: \emph{Agent Manager}, \emph{Network Scanner}, \emph{SNMP Discovery} en \emph{CVE Scanner}. Daarnaast werd een Ruckus vSZ-tab toegevoegd voor synchronisatie van access points per zone naar NetBox. Dit breidt de oorspronkelijke workflow uit met subnet-sweeps, vulnerability checks en vendor-specifieke AP-synchronisatie.
...
In de tool wordt de synchronisatie in twee stappen gedaan: eerst \emph{analyse} (dry-run/diff), daarna pas de effectieve write naar NetBox. Dit verlaagt het risico op foutieve updates.
...
\subsubsection{LLDP- en kabelafhandeling}
...
\subsection{Vendordekking en beperkingen}

De vernieuwde tool ondersteunt operationeel MikroTik, Huawei en Fortinet. Voor Ruckus access points is bijkomend een vSZ API-koppeling voorzien. Via die koppeling kunnen AP's per zone aan een NetBox-site gekoppeld worden met correcte naam, serienummer, MAC-adres, fabrikant, type en IP-adres.

\begin{listing}[H]
  \caption{Ruckus vSZ: inventarisdatadict richting NetBox}
...
Met de huidige vSZ-koppeling kunnen voor Ruckus AP's naast naam, serienummer, MAC-adres, fabrikant en type ook IP-adressen in NetBox verwerkt worden. Daardoor ondersteunt deze vendorflow zowel inventarisvelden als IP-gerelateerde documentatie.
